...
<!-- <div>
  ...
  <div class="w-screen h-screen flex flex-col bg-gray-100 overflow-hidden">
    ...
    <div class="flex items-center justify-between p-4 bg-white shadow z-10">
      <div class="flex items-center space-x-4">
        <label class="text-lg font-semibold text-gray-700" for="maxChildrenInput">
          ...
          >
            <template #person="slotProps">
              ...
              >
                ...
                <span class="font-semibold text-xl block text-gray-900 mb-1">
  ...
  <span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full w-fit">
    👥 Descendants: {{ slotProps.node.data.stats?.totalDescendants ?? 0 }}
  </span>
  <span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full w-fit">
    🌿 Non-leaf: {{ slotProps.node.data.stats?.nonLeafDescendants ?? 0 }}
  </span>
  <span class="bg-green-100 text-green-800 px-3 py-1 rounded-full w-fit">
    💰 IC Cost: ${{ slotProps.node.data.stats?.icCost?.toLocaleString() ?? 0 }}
  </span>
  <span class="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full w-fit">
    🧑‍💼 Mgmt Cost: ${{ slotProps.node.data.stats?.managementCost?.toLocaleString() ?? 0 }}
  </span>
  <span class="bg-gray-100 text-gray-800 px-3 py-1 rounded-full w-fit">
    📊 Total Cost: ${{ slotProps.node.data.stats?.totalCost?.toLocaleString() ?? 0 }}
  </span>
  <span class="bg-purple-100 text-purple-800 px-3 py-1 rounded-full w-fit">
    ⚖️ Ratio: {{
      (slotProps.node.data.stats?.icCost && slotProps.node.data.stats?.managementCost)
        ? (slotProps.node.data.stats.icCost / (slotProps.node.data.stats.managementCost || 1)).toFixed(2)
        : 'N/A'
    }}
  </span>
</div>
                ...
                <span
                  v-if="slotProps.node._fullChildren?.length > 0 && slotProps.node.children?.length === 0"
...
function limitChildren(node, max) {
  ...
  const newNode = {
    ...node,
    children: node.children.slice(0, max).map((child) => limitChildren(child, max)),
  };
  return newNode;
}
